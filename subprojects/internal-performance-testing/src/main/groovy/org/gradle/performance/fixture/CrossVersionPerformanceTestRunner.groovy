    ...
    private void runVersion(String displayName, GradleDistribution dist, File workingDir, MeasuredOperationList results) {
        ...
        def builder = GradleBuildExperimentSpec.builder()
            ...
            .invocation {
                ...
                args((this.args + ['--stacktrace', '-I', RepoScriptBlockUtil.createMirrorInitScript().absolutePath, "-D${PLUGIN_PORTAL_OVERRIDE_URL_PROPERTY}=${gradlePluginRepositoryMirrorUrl()}".toString(),
                    "-Dorg.gradle.internal.instrumentation.agent=true"]) as String[])
